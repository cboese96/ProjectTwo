...
Key.prototype.act = function(step, level) {
  /* Nothing */
}
var maxStep = 0.05;
...
var jitterSpeed = 10, jitterDist = 0.1;
...
Boost.prototype.act = function(step) {
  this.jitter += step * jitterSpeed;
  var jitterPos = Math.sin(this.jitter) * jitterDist;
  var jitterPosX = Math.cos(this.jitter) * jitterDist;
  this.pos = this.basePos.plus(new Vector(jitterPosX, jitterPos));
}
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null) {
    ...
    this.player.pos = this.player.pos.plus(new Vector(-15, 12));
  } else if (type == "boost") {
    this.actors = this.actors.filter(function(other) {
      return other != actor;
    });
...
function runGame(plans, Display) {
  ...
  alert ("WELCOME! \nCollect each coin and then the key, which is in pink, to advance to the next level. \n If you would like an added boost, just collect the purple character! \n For more fun, collide with the red character and it will teleport you!");
  startLevel(3);
}
